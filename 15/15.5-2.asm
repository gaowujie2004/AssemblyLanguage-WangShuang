; ----------- 在中断例程代码段使用数据段，注意偏移。 因为中断例程第一条代码地址是 0:204h，以往的ip=0
...
code segment 
    ...
    int9:
        ...
        jmp set_ah
        is_init: db 00010000B   ;可行的前提是int9中断例程在内存中常驻。
        set_ah:
            ; ---------------------------------------------------------------------注意：需要加上 0240h。因为int9是安装在内存0:0204h处的。 平常的程序是因为 cs:ip ，ip为零，这一次的ip=204h
            add byte ptr cs:[0204h + offset is_init-offset int9], 00010000B
            mov ah, byte ptr cs:[0204h + offset is_init-offset int9]
